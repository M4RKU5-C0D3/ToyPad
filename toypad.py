...
class ToyPad:
    INIT = [0x55, 0x0f, 0xb0, 0x01, 0x28, 0x63, 0x29, 0x20, 0x4c, 0x45, 0x47, 0x4f, 0x20, 0x32, 0x30, 0x31, 0x34, 0xf7,
    ...
    def flash(self, pad, color):
        self.color(pad, color)
        time.sleep(1)
        self.color(pad, self.OFF)
        return

    @staticmethod
    def ekey(pid, tid, action):
    ...
    def trigger(self, pid, tid, action):
        ...
        if self.ekey(pid, tid, action) in self.events:
            self.events[self.ekey(pid, tid, action)](self, pid, tid, action)
